...
const defaultState = {
  ...
  equipment: [
    { equipmentId: "BOT-001", name: "Botiquin principal", type: "Emergencia", activity: "Senderismo", status: "operativo", nextInspection: "", nextCheck: "", inspectionDate: "", maintenanceDate: "", evidence: "" },
    { equipmentId: "RAD-001", name: "Radios de comunicacion", type: "Comunicacion", activity: "Senderismo", status: "revision", nextInspection: "", nextCheck: "", inspectionDate: "", maintenanceDate: "", evidence: "" },
    { equipmentId: "CHA-001", name: "Chalecos salvavidas", type: "Seguridad acuatica", activity: "Rafting", status: "revision", nextInspection: "", nextCheck: "", inspectionDate: "", maintenanceDate: "", evidence: "" },
    { equipmentId: "CAS-001", name: "Cascos para cuatrimoto", type: "Proteccion personal", activity: "Paseo en cuatrimotos", status: "revision", nextInspection: "", nextCheck: "", inspectionDate: "", maintenanceDate: "", evidence: "" }
  ],
  policies: [
...
function loadPilotExampleData() {
  ...
  state.equipment = [
    { equipmentId: "BAL-001", name: "Balsa 6 personas", type: "Rafting", activity: "Rafting Rio Claro", status: "operativo", nextInspection: "30/06/2026", nextCheck: "30/06/2026", inspectionDate: "01/05/2026", maintenanceDate: "20/04/2026", evidence: "inspeccion_balsa_mayo.pdf" },
    { equipmentId: "CHA-001", name: "Chalecos salvavidas", type: "EPP acuatico", activity: "Rafting Rio Claro", status: "operativo", nextInspection: "30/06/2026", nextCheck: "30/06/2026", inspectionDate: "01/05/2026", maintenanceDate: "20/04/2026", evidence: "checklist_chalecos.pdf" },
    { equipmentId: "RAD-001", name: "Radios VHF", type: "Comunicacion", activity: "Caminata Bosque Alto", status: "operativo", nextInspection: "20/06/2026", nextCheck: "20/06/2026", inspectionDate: "05/05/2026", maintenanceDate: "05/05/2026", evidence: "prueba_radios.pdf" },
    { equipmentId: "CUA-001", name: "Cuatrimoto 01", type: "Vehiculo recreativo", activity: "Ruta Cuatrimotos Mirador", status: "revision", nextInspection: "", nextCheck: "", inspectionDate: "", maintenanceDate: "", evidence: "" }
  ];
  state.policies = [
...
function policyIsComplete(policy) {
  ...
  const supportOk = policy.policyLink || policy.document || policy.evidence;
...
function policyGapReason(policy) {
  ...
  if (!policy.policyLink && !policy.document && !policy.evidence) missing.push("link o soporte de poliza");
...
function activityGapItems(activityName) {
  ...
  const equipmentPending = related.equipment.filter((item) => item.status !== "operativo" || !nextEquipmentInspection(item) || String(nextEquipmentInspection(item)).toLowerCase().includes("por "));
...
function addPolicyForActivity(activityName) {
  ...
  if (!existing) state.policies.unshift({ number: `POL-${String(count).padStart(3, "0")}`, insurer: "Aseguradora por definir", coverage: `Cobertura por definir para ${activityName}`, activity: activityName, due: "", insuredValue: "", status: "pendiente", policyLink: "" });
  ...
  showActivityQuickResult(activityName, existing ? "Ya habia una poliza pendiente. Te llevo al bloque de seguro para completarla." : "Seguro agregado. Completa aseguradora, cobertura, vigencia, valor asegurado y link a poliza.", "policies");
}
  ...
  return equipmentItems.map(({ equipment, index }) => {
    ...
    return `
      <div class="equipment-edit-row">
        ...
        <label>Equipo<input data-equipment-field="${index}:name" type="text" value="${escapeHtml(equipment.name || "")}" placeholder="Balsa, casco, chaleco"></label>
        <label>Tipo / cantidad<input data-equipment-field="${index}:type" type="text" value="${escapeHtml(equipment.type || "")}" placeholder="12 unidades"></label>
        <label>Estado
        ...
        <label>Ultima inspeccion<input data-equipment-field="${index}:inspectionDate" type="date" value="${escapeHtml(dateInputValue(equipment.inspectionDate))}"></label>
        <label>Proxima inspeccion<input data-equipment-field="${index}:nextInspection" type="date" value="${escapeHtml(dateInputValue(inspectionDue))}"></label>
...
function equipmentGapReason(equipment) {
  const missing = [];
  if (!equipment.equipmentId || String(equipment.equipmentId).toLowerCase().includes("por ")) missing.push("ID/codigo");
  ...
  const nextInspection = nextEquipmentInspection(equipment);
  if (!equipment.inspectionDate) missing.push("ultima inspeccion");
  if (!nextInspection || String(nextInspection).toLowerCase().includes("por ")) missing.push("proxima inspeccion");
  if (!equipment.maintenanceDate) missing.push("ultimo mantenimiento");
...
function updateActivityEquipmentField(field) {
  ...
  if (key === "nextInspection") equipment.nextCheck = field.value;
  ...
  return policies.map(({ policy, index }) => {
    ...
    return `
      <div class="policy-edit-row">
        ...
        <label>Vigencia hasta<input data-policy-field="${index}:due" type="date" value="${escapeHtml(dateInputValue(policy.due))}"></label>
        <label class="wideish">Cobertura<input data-policy-field="${index}:coverage" type="text" value="${escapeHtml(policy.coverage || "")}"></label>
        <label>Valor asegurado<input data-policy-field="${index}:insuredValue" type="text" value="${escapeHtml(policy.insuredValue || "")}" placeholder="$"></label>
        <label class="wideish">Link a poliza<input data-policy-field="${index}:policyLink" type="url" value="${escapeHtml(policy.policyLink || policy.document || policy.evidence || "")}" placeholder="https://..."></label>
  ...
  state.equipment.forEach((equipment) => {
    ...
    if (!state.actions.some((action) => action.title === title && action.status !== "cerrada")) {
      state.actions.unshift({
        ...
        dueDate: nextEquipmentInspection(equipment) || "",
        cause: `Equipo de ${itemActivityName(equipment)} requiere ID/codigo, estado operativo, ultima inspeccion, proxima inspeccion, mantenimiento y evidencia.`,
        immediateCorrection: "",
        followUp: "",
        efficacyVerification: "",
        efficacyStatus: "pendiente",
  ...
  container.innerHTML = state.equipment.length
    ? state.equipment.map((item, index) => {
      ...
      <div class="equipment-card">
        ...
        <div class="muted">${escapeHtml(item.equipmentId || "ID por definir")} - ${escapeHtml(item.type || "Tipo por definir")} - Actividad: ${escapeHtml(activityValue)} - Proxima inspeccion: ${nextEquipmentInspection(item) || "Por programar"}</div>
        ...
        <div class="training-edit-grid">
          <label>ID / codigo<input data-equipment-main-field="${index}:equipmentId" type="text" value="${escapeHtml(item.equipmentId || "")}" placeholder="Ej. BAL-001, CAS-012, CH-023"></label>
          ...
          <label>Proxima inspeccion<input data-equipment-main-field="${index}:nextInspection" type="date" value="${escapeHtml(dateInputValue(nextEquipmentInspection(item)))}"></label>
          ...
          <label class="wideish">Descripcion / observaciones<input data-equipment-main-field="${index}:description" type="text" value="${escapeHtml(item.description || "")}" placeholder="Ej. Balsa para 6 pasajeros, revisar valvulas, piso, remos y cuerdas antes de cada salida"></label>
        </div>
      </div>`;
    }).join("")
...
function policyCoverageRows() {
  return state.activities.map((activity) => {
    ...
    if (pending.some((policy) => !policy.policyLink && !policy.document && !policy.evidence)) missing.push("link o soporte de poliza");
    ...
    const scoreParts = [
      ...
      policies.some((policy) => policy.policyLink || policy.document || policy.evidence),
...
function policyCoverageMatrixText() {
  ...
  return [
    ...
    ...policyCoverageRows().flatMap((row) => [
      ...
      `- Valor asegurado: ${row.policies.map((policy) => policy.insuredValue || "por definir").join(", ") || "ninguno"}`,
      `- Link/soporte: ${row.policies.map((policy) => policy.policyLink || policy.document || policy.evidence || "por definir").join(", ") || "ninguno"}`,
...
function renderPolicies() {
      ...
      <div class="policy-card">
        <div class="action-card-head">
          ...
          <div class="row-actions">
            ...
            <button class="secondary-button" data-remove-policy="${index}" type="button">Quitar</button>
          </div>
        </div>
        <strong>${item.number} - ${item.insurer}</strong>
        <div class="muted">${item.coverage} - Actividad: ${item.activity} - Vigencia: ${item.due || "Por definir"}${item.insuredValue ? ` - Valor: ${escapeHtml(item.insuredValue)}` : ""}</div>
        ...
        <div class="training-edit-grid">
          ...
          <label>Vigencia hasta<input data-policy-main-field="${index}:due" type="date" value="${escapeHtml(dateInputValue(item.due))}"></label>
          <label>Cobertura<input data-policy-main-field="${index}:coverage" type="text" value="${escapeHtml(item.coverage || "")}"></label>
          <label>Valor asegurado<input data-policy-main-field="${index}:insuredValue" type="text" value="${escapeHtml(item.insuredValue || "")}" placeholder="$"></label>
          <label class="wideish">Link a poliza<input data-policy-main-field="${index}:policyLink" type="url" value="${escapeHtml(item.policyLink || item.document || item.evidence || "")}" placeholder="https://..."></label>
        </div>
      </div>`;
    }).join("")
  ...
  container.querySelectorAll("[data-policy-action]").forEach((button) => {
    button.addEventListener("click", () => createPolicyAction(Number(button.dataset.policyAction)));
  });
  container.querySelectorAll("[data-remove-policy]").forEach((button) => {
    button.addEventListener("click", () => removePolicy(Number(button.dataset.removePolicy)));
  });
...
function createPolicyAction(index) {
  syncPolicyFieldsFromView(index);
  const policy = state.policies[index];
  if (!policy) return;
  const title = `Validar seguro ${policy.number || index + 1} para ${policy.activity || "actividad"}`;
  const existing = state.actions.find((action) => action.title === title && action.status !== "cerrada");
  if (!existing) {
    state.actions.unshift({
      title,
      code: "6.1.3",
      ...
      type: "preventiva",
      origin: "seguro",
      priority: policyIsComplete(policy) ? "media" : "alta",
      responsible: state.ownerName || "Responsable SGSTA",
      dueDate: "",
      cause: policyGapReason(policy),
      immediateCorrection: "Verificar vigencia, cobertura, valor asegurado y link/soporte de la poliza.",
      followUp: "",
      evidence: policy.policyLink || policy.document || policy.evidence || "",
      efficacyVerification: "",
      efficacyStatus: "pendiente",
      relatedActivity: policy.activity || "",
      sourceDetail: `Poliza ${policy.number || "sin numero"}`,
      createdAt: today()
    });
  }
  state.actionFilterActivity = policy.activity || "";
  state.routeNotice = {
    title: existing ? "Accion de seguro ya existia" : "Accion de seguro creada",
    detail: `${title}. Te llevo a Acciones para asignar responsable, fecha, seguimiento y evidencia.`
  };
  state.compliance["6.1.3"] = "en_proceso";
  recordAuditEvent({
    title: existing ? "Accion de seguro existente abierta" : "Accion de seguro creada",
    detail: title,
    code: "6.1.3",
    type: "seguro",
    actor: "humano"
...
function removePolicy(index) {
  ...
  state.compliance["6.1.3"] = policyCoverageRows().every((row) => !row.missing.length) ? "cumple" : "en_proceso";
  addMessage("agent", `Quite la poliza ${policy.number || index + 1}. Revisa si la actividad queda sin cobertura.`);
  recordAuditEvent({
    title: "Poliza eliminada",
    detail: `${policy.number || "Sin numero"} - ${policy.activity || "sin actividad"}`,
    code: "6.1.3",
    type: "seguro",
    actor: "humano"
  });
...
function buildSystemAlerts() {
  ...
  state.equipment.forEach((equipment) => {
    ...
    alerts.push({
      ...
      detail: `${equipment.name || "Equipo sin nombre"} esta ${equipment.status} y su proxima inspeccion es ${inspectionDue || "por programar"}. ${due.label}.`,
      code: "7.1",
...
function buildAgendaItems() {
  ...
  state.equipment.forEach((equipment) => {
    ...
    if (due.severity !== "baja" || equipment.status !== "operativo") {
      items.push({
        title: `Inspeccion de equipo: ${equipment.name}`,
        detail: `${equipment.type} - estado ${equipment.status}. ${due.label}.`,
        code: "7.1",
        date: inspectionDue || "Por programar",
        priority: equipment.status !== "operativo" || due.severity === "alta" ? "alta" : "media",
        origin: "equipos",
        actionTitle: `Programar inspeccion de ${equipment.name}`
      });
    }
  });
  state.policies.forEach((policy) => {
...
function buildAgentFindings() {
  ...
  const equipmentPending = state.equipment.filter((item) => item.status !== "operativo" || !nextEquipmentInspection(item) || String(nextEquipmentInspection(item)).toLowerCase().includes("por "));
  if (equipmentPending.length) {
    findings.push({
      title: "Equipos sin control operacional completo",
      detail: `${equipmentPending.length} equipo(s) requieren ID, estado operativo, inspeccion, mantenimiento o evidencia.`,
      priority: "alta",
      code: "8.1",
      actionType: "preventiva",
      origin: "equipo",
...
function addEquipment() {
  state.equipment.unshift({ equipmentId: "", name: "", type: "", description: "", activity: primaryActivityName(), status: "revision", nextInspection: "", nextCheck: "", inspectionDate: "", maintenanceDate: "", responsible: "", evidence: "" });
  state.compliance["7.1"] = "en_proceso";
  state.compliance["8.1"] = "en_proceso";
...
function addPolicy() {
  const activity = primaryActivityName();
  const count = state.policies.length + 1;
  state.policies.unshift({ number: `POL-${String(count).padStart(3, "0")}`, insurer: "Aseguradora por definir", coverage: "Cobertura por definir", activity, due: "", insuredValue: "", status: "pendiente", policyLink: "" });
  state.compliance["6.1.3"] = "en_proceso";
...
function serviceSpecificControls(activityName = "") {
  const lower = activityName.toLowerCase();
  if (lower.includes("rafting") || lower.includes("balsa") || lower.includes("rio")) {
    return [
      "- Confirmar caudal, nivel del rio, puntos de embarque/desembarque, condiciones climaticas y criterio del guia antes de iniciar.",
      "- Verificar chaleco, casco, remo, balsa, cuerda/rescate, botiquin y medio de comunicacion antes de salir.",
      "- Confirmar que el participante conoce comandos, posicion en la balsa, actuacion ante caida al agua y necesidad de seguir instrucciones.",
      "- No iniciar si el caudal, clima, estado del grupo, equipo o competencia disponible supera los criterios definidos por la empresa."
    ].join("\n");
  }
  if (lower.includes("sender") || lower.includes("caminata") || lower.includes("trek")) {
    return [
      "- Confirmar estado del sendero, clima, visibilidad, puntos de hidratacion, tiempos de recorrido y rutas de evacuacion.",
      "- Verificar calzado, hidratacion, botiquin, comunicacion, control de grupo y condiciones fisicas de los participantes.",
      "- Mantener conteo del grupo, ritmo seguro, puntos de parada y comunicacion clara sobre cambios del terreno.",
      "- No iniciar o suspender si hay tormenta, perdida de visibilidad, lesion, fatiga critica o cambio del riesgo no controlable."
    ].join("\n");
  }
  if (lower.includes("cuatrimoto") || lower.includes("moto")) {
    return [
      "- Confirmar estado de la ruta, pendientes, cruces, trafico, clima y restricciones locales antes de iniciar.",
      "- Verificar casco, instrucciones de conduccion, velocidad maxima, distancia entre vehiculos y comunicacion con guia.",
      "- Confirmar edad, habilidad minima, comportamiento seguro y aceptacion de reglas de conduccion.",
      "- No iniciar o suspender si hay falla mecanica, comportamiento inseguro, clima adverso o ruta fuera de control."
    ].join("\n");
  }
  return [
    "- Confirmar condiciones del lugar, clima, acceso, riesgos especificos, competencia del guia y equipos requeridos antes de iniciar.",
    "- Comunicar reglas de seguridad, restricciones, criterios de suspension y actuacion ante emergencia.",
    "- Mantener supervision continua y registrar novedades, incidentes, cambios de condiciones y acciones posteriores.",
    "- No iniciar o suspender cuando el riesgo supere los criterios definidos o falte personal, equipo, seguro, informacion o evidencia requerida."
  ].join("\n");
}
...
function documentDraftTemplates() {
  ...
  const profile = state.company.profileSummary || buildCompanyImplementationProfile();
  const mainActivity = state.activities[0] || {};
